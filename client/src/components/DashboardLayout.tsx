      ...
      <main className={`flex-1 ${collapsed?"ml-16":"ml-64"} transition-all duration-200`}>
        <header className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between sticky top-0 z-40 shadow-sm">
          ...
          <div className="flex items-center gap-3">
            {showBell && (<Link href="/requests"><a className="p-2 rounded-full hover:bg-slate-100 inline-block"><span className="relative inline-block"><Bell size={18} className="text-slate-600" />{pendingCount > 0 && (<span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center leading-none">{pendingCount}</span>)}</span></a></Link>)}
