...
export function DataProvider({children}:{children:React.ReactNode}){
  ...
  const[equipment,setEquipment]=useState<Equipment[]>([]);const[equipmentStock,setEquipmentStock]=useState<any[]>([]);const[siteStock,setSiteStock]=useState<any[]>([]);const[equipmentTransfers,setEquipmentTransfers]=useState<any[]>([]);const[equipmentMovements,setEquipmentMovements]=useState<any[]>([]);
  ...
  const load=useCallback(async()=>{
    ...
    setLoading(true);
    try{
      const[s,t,e,p,x,c,q,sq,ss,et,em,v,u,rq]=await Promise.all([
        api.getSites(),api.getTeams(),api.getEmployees(),
        api.getProductions(),api.getExpenses(),api.getCash(),
        api.getEquipment(),api.getEquipmentStock().catch(()=>[]),api.getSiteStock().catch(()=>[]),api.getEquipmentTransfers().catch(()=>[]),api.getEquipmentMovements().catch(()=>[]),api.getAdvances(),
        api.getUsers().catch(()=>[]),
        api.getRequests().catch(()=>[]),
      ]);
      setSites(s);setTeams(t);setEmployees(e);
      setProductions(p);setExpenses(x);setCash(c);
      setEquipment(q);setEquipmentStock(sq);setSiteStock(ss);setEquipmentTransfers(et);setEquipmentMovements(em);setAdvances(v);setAppUsers(u);setRequests(rq);
  ...
  return<DataContext.Provider value={{
    ...
    addEquipment,updateEquipment,deleteEquipment,addAdvance,deleteAdvance,updateGoldStock,equipmentStock,siteStock,equipmentTransfers,equipmentMovements,createSortie,createRetour,transferToSite,requests,addRequest,updateRequestStatus,updateRequest,
