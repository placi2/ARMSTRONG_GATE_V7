...
interface Ctx{
  ...
  requests:any[];equipmentStock:any[];siteStock:any[];equipmentTransfers:any[];equipmentMovements:any[];attendance:any[];saveAttendance:(records:any[])=>Promise<void>;createSortie:(d:any)=>Promise<any>;createRetour:(id:string,d:any)=>Promise<void>;transferToSite:(d:any)=>Promise<void>;addRequest:(r:any)=>Promise<void>;updateRequestStatus:(id:string,status:string)=>Promise<void>;updateRequest:(id:string,data:any)=>Promise<void>;
...
export function DataProvider({children}:{children:React.ReactNode}){
  ...
  const load=useCallback(async()=>{
    ...
    setLoading(true);
    try{
      const[s,t,e,p,x,c,q,sq,ss,et,em,at,v,u,rq]=await Promise.all([
        api.getSites(),api.getTeams(),api.getEmployees(),
        api.getProductions(),api.getExpenses(),api.getCash(),
        api.getEquipment(),api.getEquipmentStock().catch(()=>[]),api.getSiteStock().catch(()=>[]),api.getEquipmentTransfers().catch(()=>[]),api.getEquipmentMovements().catch(()=>[]),api.getAttendance().catch(()=>[]),api.getAdvances(),
        api.getUsers().catch(()=>[]),
        api.getRequests().catch(()=>[]),
      ]);
      setSites(s);setTeams(t);setEmployees(e);
      setProductions(p);setExpenses(x);setCash(c);
      setEquipment(q);setEquipmentStock(sq);setSiteStock(ss);setEquipmentTransfers(et);setEquipmentMovements(em);setAttendance(at);setAdvances(v);setAppUsers(u);setRequests(rq);
  ...
  return<DataContext.Provider value={{
    ...
    addEquipment,updateEquipment,deleteEquipment,addAdvance,deleteAdvance,updateGoldStock,equipmentStock,siteStock,equipmentTransfers,equipmentMovements,attendance,saveAttendance,createSortie,createRetour,transferToSite,requests,addRequest,updateRequestStatus,updateRequest,
