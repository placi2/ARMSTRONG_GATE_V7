...
async function initDB(){
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS salary_deductions(
      ...
      monthly_rate NUMERIC DEFAULT 20,
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS equipment_movements(
      ...
      date TEXT,
      status_out TEXT DEFAULT 'bon_etat',
      status_return TEXT,
      responsible_type TEXT,
      responsible_id TEXT,
      responsible_name TEXT,
      notes TEXT,
      movement_type TEXT DEFAULT 'sortie',
      created_by TEXT,
      created_at TIMESTAMPTZ DEFAULT NOW(),
      updated_at TIMESTAMPTZ DEFAULT NOW()
    );
  ...
  await pool.query(`ALTER TABLE productions ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'en_attente'`).catch(()=>{});
  await pool.query(`ALTER TABLE productions ADD COLUMN IF NOT EXISTS validated_by TEXT`).catch(()=>{});
  await pool.query(`ALTER TABLE productions ADD COLUMN IF NOT EXISTS validated_at TIMESTAMPTZ`).catch(()=>{});
...
app.post("/api/productions",auth,async(req:any,res)=>{const{id,teamId,siteId,date,weight,pricePerGram,estimatedValue}=req.body;await pool.query("INSERT INTO productions(id,team_id,site_id,date,weight,price_per_gram,estimated_value,status)VALUES($1,$2,$3,$4,$5,$6,$7,'en_attente')ON CONFLICT(id)DO NOTHING",[id,teamId,siteId,date,weight,pricePerGram,estimatedValue]);res.json({ok:true});});
...
app.put("/api/productions/:id",auth,async(req:any,res)=>{const{weight,pricePerGram,status}=req.body;const p=await pool.query("SELECT * FROM productions WHERE id=$1",[req.params.id]);const prod=p.rows[0];if(!prod)return res.status(404).json({error:"Production introuvable"});const newWeight=weight!==undefined?weight:prod.weight;const newPrice=pricePerGram!==undefined?pricePerGram:prod.price_per_gram;const newEstValue=newWeight*newPrice;await pool.query("UPDATE productions SET weight=$1,price_per_gram=$2,estimated_value=$3,status=$4,validated_by=$5,validated_at=NOW() WHERE id=$6",[newWeight,newPrice,newEstValue,status||prod.status,(req as any).user?.id||"",req.params.id]);res.json({ok:true});});
...
app.get("/api/settings",async(req,res)=>{const r=await pool.query("SELECT * FROM app_settings WHERE id=1");const s=r.rows[0]||{};res.json({goldPriceUsd:parseFloat(s.gold_price_usd)||65,goldPrice:parseFloat(s.gold_price_usd)||65,currency:s.currency||"USD",exchangeRateCdf:parseFloat(s.exchange_rate_cdf)||2800,exchangeRate:parseFloat(s.exchange_rate_cdf)||2800,companyName:s.company_name||"ARMSTRONG GATE",customLogo:s.custom_logo||null,email:s.email||null,phone:s.phone||null,address:s.address||null});});
app.put("/api/settings",auth,async(req:any,res)=>{const{goldPriceUsd,currency,exchangeRateCdf,companyName,customLogo,email,phone,address}=req.body;await pool.query("INSERT INTO app_settings(id,gold_price_usd,currency,exchange_rate_cdf,company_name,custom_logo,email,phone,address)VALUES(1,$1,$2,$3,$4,$5,$6,$7,$8)ON CONFLICT(id)DO UPDATE SET gold_price_usd=$1,currency=$2,exchange_rate_cdf=$3,company_name=$4,custom_logo=$5,email=$6,phone=$7,address=$8,updated_at=NOW()",[goldPriceUsd,currency,exchangeRateCdf,companyName,customLogo||null,email||null,phone||null,address||null]);res.json({ok:true});});
...
// Routes mouvements équipements (sorties/retours)
app.get("/api/equipment-movements",auth,async(req:any,res)=>{const u=req.user;const r=u.siteId?await pool.query("SELECT * FROM equipment_movements WHERE site_id=$1 ORDER BY created_at DESC",[u.siteId]):await pool.query("SELECT * FROM equipment_movements ORDER BY created_at DESC");res.json(r.rows.map(c));});
...
app.put("/api/equipment-movements/:id/retour",auth,async(req:any,res)=>{const{qtyGood,qtyCasse,qtyManquant,responsibleType,responsibleId,responsibleName,notes}=req.body;const mv=await pool.query("SELECT * FROM equipment_movements WHERE id=$1",[req.params.id]);const m=mv.rows[0];if(!m)return res.status(404).json({error:"Mouvement introuvable"});const qtyReturned=(parseFloat(qtyGood)||0)+(parseFloat(qtyCasse)||0)+(parseFloat(qtyManquant)||0);await pool.query("UPDATE equipment_movements SET qty_returned=$1,status_return=$2,responsible_type=$3,responsible_id=$4,responsible_name=$5,notes=$6,updated_at=NOW() WHERE id=$7",[qtyReturned,parseFloat(qtyCasse)>0?"casse":parseFloat(qtyManquant)>0?"manquant":"bon_etat",responsibleType||null,responsibleId||null,responsibleName||null,notes||null,req.params.id]);await pool.query("UPDATE equipment_site_stock SET qty_available=qty_available+$1,qty_on_terrain=GREATEST(0,qty_on_terrain-$2),updated_at=NOW() WHERE equipment_id=$3 AND site_id=$4",[parseFloat(qtyGood)||0,qtyReturned,m.equipment_id,m.site_id]);if((parseFloat(qtyCasse)||0)>0||(parseFloat(qtyManquant)||0)>0){const expId=`EX${Date.now()}`;const unitVal=parseFloat(m.unit_value)||0;const qtyBad=(parseFloat(qtyCasse)||0)+(parseFloat(qtyManquant)||0);await pool.query("INSERT INTO expenses(id,team_id,site_id,category,amount,date,comment)VALUES($1,$2,$3,$4,$5,NOW()::date::text,$6)ON CONFLICT DO NOTHING",[expId,m.team_id,m.site_id,"Équipement",unitVal*qtyBad,`${parseFloat(qtyCasse)>0?"Casse":"Perte"} — ${m.equipment_name} x${qtyBad} — responsable: ${responsibleName||responsibleType||"inconnu"}`]);}if((parseFloat(qtyGood)||0)<parseFloat(m.qty_out)&&responsibleType==="employee"&&responsibleId){const stockRow=await pool.query("SELECT unit_value FROM equipment_site_stock WHERE equipment_id=$1 AND site_id=$2",[m.equipment_id,m.site_id]);const unitVal=parseFloat(stockRow.rows[0]?.unit_value)||0;const qtyBad=(parseFloat(qtyCasse)||0)+(parseFloat(qtyManquant)||0);const totalDebt=unitVal*qtyBad;const monthlyRate=totalDebt<=500?20:30;const dedId=`DED${Date.now()}`;await pool.query("INSERT INTO salary_deductions(id,employee_id,employee_name,equipment_name,amount_total,amount_paid,monthly_rate,status)VALUES($1,$2,$3,$4,$5,0,$6,'en_cours')ON CONFLICT DO NOTHING",[dedId,responsibleId,responsibleName||"",m.equipment_name,totalDebt,monthlyRate]);}res.json({ok:true});});
app.get("/api/salary-deductions",auth,async(req:any,res)=>{const u=req.user;const r=u.role==="chef_equipe"||u.role==="directeur"?await pool.query("SELECT * FROM salary_deductions WHERE employee_id IN (SELECT id FROM employees WHERE team_id=$1) ORDER BY created_at DESC",[u.teamId||""]):await pool.query("SELECT * FROM salary_deductions ORDER BY created_at DESC");res.json(r.rows.map(c));});
app.put("/api/salary-deductions/:id/pay",auth,async(req:any,res)=>{const{amount}=req.body;const sd=await pool.query("SELECT * FROM salary_deductions WHERE id=$1",[req.params.id]);const d=sd.rows[0];if(!d)return res.status(404).json({error:"Déduction introuvable"});const newPaid=parseFloat(d.amount_paid)+parseFloat(amount);const newStatus=newPaid>=parseFloat(d.amount_total)?"soldee":"en_cours";await pool.query("UPDATE salary_deductions SET amount_paid=$1,status=$2,updated_at=NOW() WHERE id=$3",[newPaid,newStatus,req.params.id]);res.json({ok:true});});
app.delete("/api/salary-deductions/:id",auth,async(req:any,res)=>{await pool.query("DELETE FROM salary_deductions WHERE id=$1",[req.params.id]);res.json({ok:true});});
